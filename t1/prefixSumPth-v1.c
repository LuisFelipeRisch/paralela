...
void *prefixSum(void *ptr)
{
	...
	int last = (myIndex + 1) == nThreads ? nTotalElements : (myIndex + 1) * nElements;
	last--;
	...
	while (true)
	{
		...
		OutputVector[first] = InputVector[first] + partial;
...
void ParallelPrefixSumPth(const TYPE *InputVec,
													const TYPE *OutputVec,
													long numberTotalElmts,
													int numberThreads)
{
	///////////////// INCLUIR AQUI SEU CODIGO da V1 /////////

	static int initialized = 0;
	nTotalElements = numberTotalElmts;
	nThreads = numberThreads;

	if (!initialized)
	{
		pthread_barrier_init(&myBarrier, NULL, numberThreads);
		...
		for (int i = 1; i < numberThreads; i++)
		{
			...
			pthread_create(&Thread[i], NULL,
										 prefixSum, &my_thread_id[i]);
		}

		initialized = 1;
	}

	prefixSum(&my_thread_id[0]);
